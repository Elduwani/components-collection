...
export function Board({ initialSquare }) {
    const ref = useRef()
    const [board] = useState(grid(8)) //8*8 grid;
    const [dimension, setDimension] = useState(undefined)
    const [selectedSquare, setSelectedSquare] = useState(initialSquare);
    ...
    return (
        <div ref={ref} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', minWidth: 340 }}>
            ...
            // style={{ width: scale.current, height: scale.current }}
            >
                {board.map((row, i) => {
                    const even = i % 2 === 0;
